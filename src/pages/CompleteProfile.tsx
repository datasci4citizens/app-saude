...
export default function CompleteProfile() {
  ...
  if (!userType) {
    return (
      <div style={containerStyle}>
        ...
        <button onClick={() => setUserType('person')} style={{ marginRight: '10px' }}>Pessoa</button>
  ...
  return (
    <div style={containerStyle}>
      <p style={{ backgroundColor: '#fff3cd', padding: '10px', borderRadius: '5px', border: '1px solid #ffeeba', color: '#856404' }}>
        ⚠️ Campos de teste, ainda há alguns faltando.
      </p>

      <form onSubmit={handleSubmit}>
        <h3>{userType === 'person' ? 'Cadastro de Pessoa' : 'Cadastro de Profissional'}</h3>

        <div style={fieldStyle}>
          <label style={labelStyle}>Nome Social (opcional)</label>
          <input name="social_name" onChange={handleChange} style={inputStyle} />
        </div>
        ...
        {userType === 'person' ? (
          <>
            <div style={fieldStyle}>
              ...
              <input type="date" name="birth" onChange={handleChange} style={inputStyle} />
            </div>
            <div style={fieldStyle}>
              <label style={labelStyle}>Altura (m)</label>
              <input type="number" step="0.01" name="height" onChange={handleChange} style={inputStyle} />
            </div>
            <div style={fieldStyle}>
          ...
          <>
            <div style={fieldStyle}>
              ...
              <input type="email" name="professional_email" onChange={handleChange} style={inputStyle} />
            </div>
            <div style={fieldStyle}>
              <label style={labelStyle}>Registro Profissional</label>
              <input type="text" name="professional_registration" onChange={handleChange} style={inputStyle} />
            </div>
          </>
        )}

        <button type="submit" style={{ marginTop: '20px', padding: '10px 20px', backgroundColor: '#007bff', color: 'white', border: 'none', borderRadius: '4px' }}>
          Enviar
        </button>
      </form>
    </div>
