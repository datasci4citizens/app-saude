...
const NewReminder: React.FC = () => {
  ...
  const [repeatType, setRepeatType] = useState<'Hour' | 'Day' | 'Week' | 'Month' | 'Year'>('Week');
  ...
  return (
    <form onSubmit={handleSubmit} className="p-4 bg-primary min-h-screen font-inter">
      ...
      {/* Time Picker */}
      <div className="mb-4 relative">
        <label className="block text-sm font-inter font-light text-gray2 mb-1">Horário:</label>
      ...
      <div className="mb-4 relative flex flex-col w-full">
        <label className="block text-sm font-inter font-light text-gray2 mb-1">Repetição:</label>
        <div className="flex justify-between w-full rounded-3xl border-1.5 border-typography overflow-hidden h-8">
          {['Hour', 'Day', 'Week', 'Month', 'Year'].map((type, index, array) => (
            <React.Fragment key={type}>
              <button
                type="button"
                onClick={() => setRepeatType(type as 'Hour' | 'Day' | 'Week' | 'Month' | 'Year')}
              ...
              {index < array.length - 1 && <span className="w-px h-5 bg-typography self-center" />}
            </React.Fragment>
          ))}
        </div>
      </div>
            ...
            {Object.keys(repeatDays).map((day) => (
              <label key={day} className="cursor-pointer">
                <Checkbox
                  ...
                  onCheckedChange={() => handleCheckboxChange(day as keyof typeof repeatDays)}
                  height="h-7"
                  width="w-7"
                  radius="rounded-md"
                >
      ...
      {/* Submit Button */}
      <Button type="submit" variant="orange" className="w-full mt-4 font-inter font-bold">
        CONTINUAR
      </Button>
    </form>
  );
